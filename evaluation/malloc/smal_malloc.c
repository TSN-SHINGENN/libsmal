...
/**
 * @fn void *smal_malloc_init(void *const buf, const uint8_t bufsize)
 * @brief mallocを扱えるように初期化します。
 * @param buf バッファポインタ
 * @param bufsize　バッファサイズ
 **/
void *smal_malloc_init(void *const buf, const uint8_t bufsize)
{
    smal_mallocater_init_obj(&_smal_mallocater_heap_obj);
}
...
void *smal_malloc(const uint8_t size)
{
    ...
    return smal_mallocater_alloc_with_obj( o, size);
}
...
void smal_free(void *const ptr)
{
    ...
    smal_mallocater_free_with_obj(o, ptr);
}
